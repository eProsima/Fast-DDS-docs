.. _api_reference:

API Reference
=============

Fast RTPS, as a DDS standard implementation, exposes the DDS Data-Centric Publish-Subscribe (DCPS) Platform Independent
Model (PIM) API, as specified in the `DDS specification <https://www.omg.org/spec/DDS/1.4/PDF>`_.
Furthermore, is also gives the user the possibility to directly interact with the underlying Real-time
Publish-Subscribe (RTPS) API that DDS implements for wired communications, as specified in the
`RTPS standard <https://www.omg.org/spec/DDSI-RTPS/2.2/PDF>`_.


.. toctree::

   /fastdds/api_reference/includes/dds_pim/dds_pim
