...
* The application indicates which relevant information it wants to get, by means of |api_pim_condition|
  objects (|api_pim_guardcondition|, |api_pim_statuscondition|, or |api_pim_readcondition|)
  and attaching them to a |api_pim_waitset| via the |WaitSet::attach_condition-api| call.
* It then waits on that |api_pim_waitset| via the |WaitSet::wait-api|
  call until the trigger value of one or several |api_pim_condition| objects become true.
* It then uses the result of the |WaitSet::wait-api| (i.e., the list of |api_pim_condition| objects with
  ...
  * Directly |DataReader::read_w_condition-api|/|DataReader::take_w_condition-api| on the DataReader with the |api_pim_condition| as a parameter,
    when it is a ReadCondition

* When a Condition is no longer relevant it can be detached from a |api_pim_waitset| via the
...
Calling the |WaitSet::wait-api| operation on the |api_pim_waitset| will block the calling thread
...
The sensitivity of the |api_pim_statuscondition| to a particular communication status is controlled by the
...
Once we have trigger_value == true on a condition, it may wake up the attached |api_pim_waitset|.
The condition transitioning to trigger_value == false does not necessarily ‘unwakeup’ the |api_pim_waitset|,
as ‘unwakening’ may not be possible in general.
The consequence is that an application blocked on a |api_pim_waitset| may return from the wait with a list
...
This is unavoidable if multiple threads are concurrently waiting on separate |api_pim_waitset| objects and
