...
Fast DDS implements the SIMPLE discovery protocol to provide compatibility with the
...
- **Simple Participant Discovery Protocol (SPDP):** specifies how DomainParticipants discover each other in the network;
  it announces and detects the presence of DomainParticipants within the same domain.
...
+------------------------------+---------------------------------------------------------------------------------------+
| :ref:`Simple Initial Peers`  | A list of DomainParticipant's IP/port pairs to which the SPDP announcements are sent. |
+------------------------------+---------------------------------------------------------------------------------------+
...
`RTPS standard <https://www.omg.org/spec/DDSI-RTPS/2.2/PDF>`_ simple discovery mechanism requires the DomainParticipants
to send announcements of their presence in the domain.
...
``leaseDuration_announcementperiod`` period (not the ``initial_announcements.period``).
...
|                             | It allows the creation of only DataWriter discovery related |br|  |          |         |
|                             | EDP endpoints.                                                    |          |         |
+-----------------------------+-------------------------------------------------------------------+----------+---------+
