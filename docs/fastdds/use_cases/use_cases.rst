...
+ :ref:`use-case-statistics-from-dedicated-interface`.
  ...
  interfaces.
  For instance, a user might need to ensure that :ref:`statistics information <statistics_module>` is
  received exclusively through a subnetwork interface that is dedicated to monitoring tasks, while all other user data
  intended for different participants is sent through alternate network interfaces.
  This setup allows users to effectively manage network traffic and ensures that sensitive monitoring data remains
  isolated within a specific subnetwork.
