...
this library is applied. That is why this section defines the implementations for the typical use cases
presented when using the RTPS protocol for communications.

+ :ref:`use-case-fast-rtps-over-wifi`. Presents the case for using Fast RTPS in scenarios where discovery through
  multicast communication is a challenge. To address this problem, the use of the initial peers list by which the
  address-port pairs of the remote participants are defined is presented (See :ref:`use-case-initial-peers`).
  It also specifies how to disable the
...
+ :ref:`fastrtps_ros2`. As the default middleware implementation in the
  `OSRF <https://www.openrobotics.org/>`_ `ROS 2 <https://index.ros.org/doc/ros2/>`_, this tutorial is an explanation
  of how to take full advantage for Fast-RTPS wide set of capabilities in a ROS 2 project.
